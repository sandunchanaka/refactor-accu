...
" . $myquery . "
				...
				echo $err_msg .= "<font color=red>".$GONX["ignoredtables"]." ".$val[0]." - ".$GONX["reservedwords"].".</font><br>\n";
			}
        } 
        ...
        echo $err_msg = "<font color=green>".$GONX["backup"]." ".$this->dbName." ".$GONX["iscorrectcreat"]." : ".$this->backupdir."/$fname</font>";
	}
...
" . $myquery . "
	...
	//					$contents = bzread($bz, filesize($this->backupdir."/".$bfile)*1000); // just a hack coz feof doesn't wrk for me
				...
				return $err_msg = "<font color=green> $bfile ".$GONX["iscorrectimport"]." </font>";
			} else return FALSE;
		}
...
//					$contents = bzread($bz, filesize($this->backupdir."/".$bfile)*1000); // just a hack coz feof doesn't wrk for me
			...
			echo $err_msg = "<font color=green> $bfile ".$GONX["iscorrectimport"]." </font>";
	...
	function importfromfile(){
		...
		if ($bfile["error"]==0) { // File existe, import it
			...
			echo $err_msg = "<font color=green> ".$bfile["name"]." ".$GONX["iscorrectimport"]." </font>";
		} else {	// Erronous file, read dir, and list available backup file.
...
\t<tr>
	...
	function delete($_fname){
		if (is_file($this->backupdir."/".$_fname)) {
			...
			echo $err_msg = "<font color=green> Backup file $_fname is correctly removed </font>";
		} else echo $err_msg = "<font color=red> Error while removing backup file $_fname</font>";
	}
