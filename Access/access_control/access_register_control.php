	...
	switch($mode)
	{
		case 'reg':
			...
			$availability =register::check_available($text_reg_no);
			 ...
			 {
			  $register = register::org_data($text_name,$text_district,$text_name_credit_coop,$text_address,$text_address_city,$text_address_country,$text_reg_no,$text_telephone,$text_fax,$text_email,$txt_pass,$select_operation,$text_employee_male,$text_employee_female,$text_establish,$text_service,$text_share,$text_deposite,$text_savings,$text_loan,$text_reserve,$text_assest);
			 
			  if($register==true)
			  {
			   echo "<script type=\"text/javascript\">document.location.replace('../access_branding.php?msg=7');</script>";
			  }
			  else
			  {
			   echo "<script type=\"text/javascript\">document.location.replace('../access_register.php?msg=4');</script>";
			  }
			 }
			 
			 else
			 {
			  echo "<script type=\"text/javascript\">document.location.replace('../access_register.php?msg=8');</script>";
			 }
