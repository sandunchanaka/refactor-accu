<?php
	...
	{	
		...
		{
			$sql="SELECT * FROM cumi_category WHERE CatCode='".$mi_cate_code."'";
		...
		{
			$sql="SELECT * FROM cumi_indicators WHERE CatID='".$mi_cateid."'";
			$data = CDBCon::GetInstance()->GetAll($sql);
			return $data;
		} 
		...
		{
			$sql_insert="INSERT INTO cumi_marks VALUES('".$comp_id."','".$MarkOption."','".$CatID."','".$CatCode."','".$Marks."','".$Year."','".$Date."','".$UserID."','".$Upd."')";
		...
		{
			$pa=md5($pass);
			$sql="SELECT * FROM cumi_register_data where user='".$name."' AND pass='".$pa."'";
			//echo $sql;exit;
			$data = CDBCon::GetInstance()->GetRow($sql);
			return $data;
		} 
		...
		{
			$sql="SELECT * FROM cumi_marks WHERE comp_id='".$comp_id."'  AND Year='".$Year."' AND  CatCode='".$CatCode."'";
			//echo $sql;exit;
			$data = CDBCon::GetInstance()->GetRow($sql);
			return $data;
		}
		...
		{
		 $sql="SELECT DISTINCT CatCode FROM cumi_marks WHERE comp_id='".$comp_id."'  AND Year='".$Year."' ";
		 //echo $sql;exit;
		 $data = CDBCon::GetInstance()->GetAll($sql);
		 return $data;
		}
		...
		function get_mi_category_name($mi_cate_code)
		  {
		   $sql="SELECT CatProsp FROM cumi_category WHERE CatCode='".$mi_cate_code."'";
		   //echo $sql;exit;
		   $data = CDBCon::GetInstance()->GetRow($sql);
		   return $data;
		  } 
		  ...
		  {
		   $sql="SELECT sum(Marks)as total FROM cumi_marks WHERE Year='".$Year."' AND comp_id='".$comp_id."' AND CatCode='".$CatCode."'";
		   $data = CDBCon::GetInstance()->GetRow($sql);
		   return $data;
		  }
