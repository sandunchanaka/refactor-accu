		    ...
		    <div id="content" class="print-area">
		      ...
		      <div class="content-container"> <span class="clearfix"></span>
		        ...
		        <div class="content-body content-body-nobg">
		          <p>&nbsp;</p>
		          <?php 
			$membertype =  Member::getMemberTypeDiscription();
			for($a=0;$a<count($membertype);$a++){
			$row = each($membertype);			
			  ?>
                  <h2 class=""><span><a href="members.php?mem_type_id=<?php echo $row["value"]["member_type_id"];?>&amp;mem_type=<?php echo $row["value"]["member_type_name"]; ?>&page=4<?php echo $a+2; ?>"><?php echo $row["value"]["member_type_name"]; ?></a></span>&nbsp;&nbsp;
                  <a href="members.php?mem_type_id=<?php echo $row["value"]["member_type_id"];?>&amp;mem_type=<?php echo $row["value"]["member_type_name"]; ?>&page=4<?php echo $a+2; ?>" class="linkTest">(
				<?php $mem_tot =  Member::MemberCount($row["value"]["member_type_id"]);
				if($mem_tot["total"] ==1){
					  		echo $mem_tot["total"]."&nbsp;&nbsp; Member"; 
				}elseif($mem_tot["total"] > 1){
					echo $mem_tot["total"]."&nbsp;&nbsp; Members"; 
				} ?>)
                </a>
                  </h2>
		          <div class="">
		            ...
		            <p><?php echo $row["value"]["member_type_description"]; ?></p>
                    <br>
	              </div>
                  <?php } ?>
		        </div>
