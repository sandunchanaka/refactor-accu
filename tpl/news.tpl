...
<h2 class="home"><a href="#">News Highlights</a><a href="#" class="icons feed-icon">News RSS Feed</a></h2>
    
<div class="news-listing meta-dynamic-content-container" style="height: 503px;">
	<ul class="listing absolute" style="height: 503px;">
   <?php 
  			$news = News::GetNews();
			for($an = 0;$an<count($news);$an++){
			$row_n = each($news);	
		 ?>
    <li class="news news-with-image first" style="width: 350px; display: list-item;">
        ...
        <div class="details"><span class="date" title="<?php echo $row_n["value"]['news_date'];?>"><?php  
			$getdate = strtotime($row_n["value"]['news_date']);
            echo $getdate;
			//echo date("l F d, Y", $getdate); ?> <!--////Monday December 17, 2012--></span>
        <h4><a href="#" title="<?php $row_n["value"]['news_name']; ?>"><?php echo $row_n["value"]['news_name'];?></a></h4>
     ...
     <?php } ?>
        ...
        <li class="meta-all-items">
			<a href="news-highlights.php" title="View all news items">View All</a>
		</li>
	</ul> <!-- .pager -->
</div>
